...
func (l *TextFormatter) Format(e *event.Event) []byte {
	...
	if !simple {
		if l.flags&CLcolor != 0 {
			spew.Fprintf(b, "\x1b[%dm%s\x1b[0m ", l.levelColor(e.Level), e.Level)
		} else {
			spew.Fprintf(b, "%s ", e.Level)
		}
	}

	if len(e.Topic) > 0 {
		if l.flags&CLcolor != 0 {
			spew.Fprintf(b, "\x1b[92m%s\x1b[0m ", e.Topic)
		} else {
			spew.Fprintf(b, "%s ", e.Topic)
		}
	}
	...
	if !simple && l.flags&Lframes == 0 {
		if len(e.Pc) > 0 {
			...
			if l.flags&Llongfile != 0 {
				spew.Fprintf(b, "%s:%d: ", f.File, f.Line)
			} else {
				spew.Fprintf(b, "%s:%d: ", path.Base(f.File), f.Line)
			}
		} else {
			spew.Fprint(b, "???:0: ")
		}
	...
	if len(e.Args) > 0 {
		if len(e.Fmt) > 0 {
			spew.Fprintf(b, e.Fmt, e.Args...)
		} else {
			spew.Fprint(b, e.Args...)
		}
	} else {
		b.WriteString(e.Fmt)
	}
	...
	if l := e.Fields.Length(); l > 0 {
		...
		for i = 0; i < l; i++ {
			...
			f := field.(event.Field)
			spew.Fprintf(b, " %s=%v", f.Name, f.Value)
		}
	}
